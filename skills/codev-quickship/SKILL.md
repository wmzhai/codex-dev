...
`codev-quickship` 是人工验证通过后的统一收尾 skill。它基于当前 task 的已确认结果，补齐最终 task 记录、归档任务、同步任务相关文档，并在每次收尾时同步根目录 `VERSION` 与已有 `CHANGELOG`：如果用户未显式指定版本，则默认按四段版本号 `x.y.z.w -> x.y.z.(w+1)`，也就是把最后一位加一；如果用户显式指定目标版本，则按指定版本写入。随后它完成“提交并推主干”：如果当前在分支上，就先把分支收成稳定状态再合并进主干；如果已经在主干上，就直接在主干上提交并推送。若 task 明确映射到 GitHub issue，它还要在主干 push 成功后先向对应 issue 追加一条本轮实际工作摘要，再关闭对应 issue。它不走标准 PR / release 链路，也不调用 `$ship`、`$land-and-deploy`、`$document-release`。
...
- 根目录 `VERSION` 已初始化且为单个可稳定解析的四段纯数字版本号。
...
7. 同步仓库里的版本工件：
   ...
   - 如果用户未显式指定版本，则默认按四段版本号 `x.y.z.w -> x.y.z.(w+1)`，也就是把最后一位加一。
...
8. 根据当前所在分支决定 quickship 路径：
   ...
   - 提交信息必须采用 `type: 具体工作摘要 (vX.Y.Z.W)` 形式；工作摘要要先写清这次做了什么，版本号放在最后的括号里，task 标识如果需要保留，也写进摘要里而不要放最前面。
9. 直接推送主干：
   - 把本地主干推到远端主干。
   - 如果推送失败、被保护规则拦截或权限不足，明确报告并停止，不要改走其他发布技能。
10. 如果 task 明确映射到一个或多个 GitHub issue，且主干 push 已成功：
...
- quickship 在未显式指定版本时，默认把四段版本号的最后一位加一。
